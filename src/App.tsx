...
import { ExternalLink, CheckCircle2, Circle, Star, ChevronDown, Copy, Check, Share2, X } from 'lucide-react';
import QRCode from 'qrcode';
import { reviewsData, availableBatches, getReviewByProfileAndBatch } from './data/reviews';
...
},
{
  id: 'divine-1',
  name: 'Divine',
  url: 'https://g.page/r/CQw5uHEwjNpJEAE/review',
  reviewCount: 1,
   display: true
}
];
...
function App() {
  ...
  const [showIndividualShare, setShowIndividualShare] = useState<string | null>(null);
  ...
  const toggleCompletion = (id: string) => {
    setProfiles(prev => 
      prev.map(profile => 
        profile.id === id ? { ...profile, completed: !profile.completed } : profile
      )
    );
  };
  ...
  const progressPercentage = profiles.length > 0 ? (completedCount / profiles.length) * 100 : 0;
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 px-3 py-4">
        ...
        <div className="text-center mb-6">
          <div className="flex items-center justify-center mb-4">
            ...
            <h1 className="text-xl font-bold text-gray-800">GMB Profile Review</h1>
          </div>
          ...
          <div className="flex items-center justify-center space-x-3">
            ...
            >
              <span>{availableBatches.find(b => b.id === selectedBatch)?.name}</span>
              <ChevronDown className={`w-4 h-4 transition-transform ${showBatchDropdown ? 'rotate-180' : ''}`} />
            </button>
                ...
                {availableBatches.map((batch) => (
                  <button
                    ...
                    className={`w-full text-left px-4 py-2 text-sm hover:bg-gray-50 transition-colors ${
                      selectedBatch === batch.id ? 'bg-blue-50 text-blue-600 font-medium' : 'text-gray-700'
                    }`}
                  >
                    {batch.name}
                  </button>
      ...
      .map((profile, index) => (
        ...
        >
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center space-x-2">
                    ...
                    <button
                      onClick={() => generateShareOptions(profile.url, profile.id)}
                ...
                <div className="mt-3 p-3 bg-gray-50 rounded-lg">
                  <p className="text-xs text-gray-600 mb-1">Review for {availableBatches.find(b => b.id === selectedBatch)?.name}:</p>
          ...
          {completedCount === profiles.length && profiles.length > 0 && (
            <div className="mt-6 p-4 bg-green-50 border-2 border-green-200 rounded-xl">
              ...
              <p className="text-green-700 mt-2 text-sm">
                Excellent work! You've successfully reviewed all {profiles.length} GMB profiles for {availableBatches.find(b => b.id === selectedBatch)?.name}. 
            ...
            >
              ...
              <div className="p-6">
                ...
                <div className="space-y-3">
                  <button
                    onClick={() => {
                      const profile = profiles.find(p => p.id === showIndividualShare);
                      if (profile) {
        ...
        {showQRCode && (
          ...
          >
            ...
            >
              ...
              {/* Modal Content */}
              <div className="p-6">
                <h3 className="text-lg font-bold text-gray-800 mb-2 text-center">Share Review Page</h3>
                <p className="text-sm text-gray-600 mb-4 text-center">
                ...
                <div className="space-y-3">
                  <button
                    onClick={() => copyLinkToClipboard(window.location.href, 'main')}
                    className={`w-full flex items-center justify-center space-x-2 rounded-lg px-4 py-3 font-medium transition-colors ${
                      copiedLink === 'main'
                        ? 'bg-green-600 text-white'
                        : 'bg-blue-600 hover:bg-blue-700 text-white'
        ...
        <div className="mt-6 text-center">
          ...
          <p className="text-gray-500 text-sm">
            Reviews are automatically copied to clipboard when you click the copy button or profile link.
          </p>
          )}
        </div>
    </div>
